...
import type { PageData, PageElement, DragItem } from "./types"
...
export default function WebsiteBuilder() {
  ...
  const getDefaultContent = (type: string) => {
    switch (type) {
      ...
      case "paragraph-full":
        return { text: "This is a full width paragraph. Click edit to customize." }
      case "paragraph-left-image":
      case "paragraph-right-image":
        return { text: "This is a paragraph with an image.", imageSrc: "/placeholder.svg?height=128&width=192" }
      case "image":
  ...
  const handleElementUpdate = useCallback((id: string, content: any) => {
    console.log("🔄 Website Builder - Updating element:", id, "with content:", content)
    setPageData((prev) => {
      const newElements = prev.elements.map((el) => {
        if (el.id === id) {
          const updatedElement = { ...el, content: { ...content } }
          console.log("✅ Website Builder - Updated element:", updatedElement)
          return updatedElement
        }
        return el
      })
      const newPageData = { ...prev, elements: newElements }
      console.log("📊 Website Builder - New page data:", newPageData)
      return newPageData
    })
  }, [])
  ...
  const handleSave = async () => {
    setIsSaving(true)
    console.log("Website Builder - Saving page data:", JSON.stringify(pageData, null, 2))
    try {
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        <div className="bg-white border-b p-4 flex justify-between items-center">
          <h1 className="text-xl font-semibold">Preview: {pageData.meta.title}</h1>
          <Button onClick={() => setIsPreview(false)}>Exit Preview</Button>
        </div>
          ...
          <div className="flex-1 flex flex-col">
            <div className="bg-white border-b p-4 flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  onClick={() => {
                    console.log("🐛 DEBUG - Current page data:", pageData)
                    pageData.elements.forEach((el, i) => {
                      console.log(`🐛 DEBUG - Element ${i}:`, el)
                    })
                  }}
                  variant="outline"
                  size="sm"
            ...
            <DroppableCanvas onDrop={handleDrop} className="flex-1 p-6">
              <div className="max-w-6xl mx-auto space-y-4">
                {pageData.elements.length === 0 ? (
                  <div className="text-center py-20 text-gray-500">
                    <p className="text-lg">Drag components from the sidebar to start building your page</p>
                  </div>
                ) : (
      ...
      {isMobile && (
        <div className="flex-1 flex flex-col">
          <div className="bg-white border-b p-4 flex justify-between items-center">
            ...
            <Button onClick={() => setIsPreview(true)} variant="outline" size="sm">
          ...
          <div className="flex-1 p-4 overflow-y-auto">
            <div className="space-y-4 pb-20">
              {pageData.elements.length === 0 ? (
                <div className="text-center py-20 text-gray-500">
                  <p className="text-base">Tap the + button to add components</p>
                </div>
              ) : (
                pageData.elements.map((element, index) => (
                  <EditableElement
          ...
          <MobileSaveButton onClick={() => setShowMobileSaveDialog(true)} isSaving={isSaving} />
          <MobileSaveDialog
