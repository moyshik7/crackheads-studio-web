      ...
      <div className="p-6 rounded-lg" style={{ backgroundColor: theme?.colors.background || "#f9fafb" }}>
        ...
        <div className="flex gap-4 overflow-x-auto pb-4">
          {sampleCards.map((card) => (
            <div key={card.id} className="flex-shrink-0 w-80 bg-white rounded-lg shadow-sm border">
              <img
      ...
      <div className={`grid ${getGridClass()} gap-4`}>
        {cards.map((card) => (
          <div key={card.id} className="bg-white rounded-lg shadow-sm border overflow-hidden">
            ...
            <div className="p-4">
              <h3 className={`font-semibold mb-2 ${type === "6-grid" ? "text-sm" : "text-lg"}`}>{card.title}</h3>
              <p className={`text-gray-600 mb-3 ${type === "6-grid" ? "text-xs" : "text-sm"}`}>{card.description}</p>
              <button
                className={`px-3 py-1 rounded text-white font-medium ${type === "6-grid" ? "text-xs" : "text-sm"}`}
                style={{ backgroundColor: theme?.colors.primary || "#3b82f6" }}
              >
                {card.action}
              </button>
